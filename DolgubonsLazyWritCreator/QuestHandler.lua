...
local function HandleChatterBegin(eventCode, optionCount)
	...
	-- If not on crafter, do not auto accept
	--if not ArmoryBuildDisplay.GetArmoryBuildName(ArmoryBuildDisplay.GetVar("lastBuildID")):find("craft") then return end
    ...
    for i = 1, optionCount do
	    ...
	    then
	    	if not WritCreater:GetSettings().autoAccept then return end
			if not ArmoryBuildDisplay.GetArmoryBuildName(ArmoryBuildDisplay.GetVar("lastBuildID")):find("craft") 
			then 
				CHAT_SYSTEM:AddMessage("You're not on your crafter, dummy!")
			return end
